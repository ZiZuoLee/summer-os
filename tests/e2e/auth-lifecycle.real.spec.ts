...
type InbucketMessageSummary = { id: string };
  ...
  await expect
    .poll(
      async () => {
        ...
        const messages = (await response.json()) as InbucketMessageSummary[];
        messageId =
          messages.find((message) => !seenIds.has(message.id))?.id ?? "";
        return Boolean(messageId);
      },
      {
        message: "Supabase Auth email did not arrive in Inbucket",
        timeout: 45_000,
...
function emailActionUrl(message: InbucketMessage) {
  const source = [
    message.body?.html,
    message.body?.text,
    message.html,
  ...
  expect(
    url,
    "Inbucket message did not contain a Supabase verification URL",
  ).toBeTruthy();
  return url!;
}
...
test.describe("真实 Supabase 与 Inbucket 认证生命周期", () => {
  test.describe.configure({ mode: "serial" });
  test.skip(
    !realAuthEnabled,
    "需要 E2E_REAL_AUTH=true、本地 Supabase、Inbucket 和 Cloudflare Turnstile 测试密钥。",
  );
  ...
  test("注册、验证、引导、持久化、重置密码、导出和删除", async ({
    ...
    try {
      ...
      await test.step("reset through Inbucket and use the new password", async () => {
